...
const ReviewerPeer = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (validation()) {
            if (data.status < 1) {
                axios.post('http://localhost:5000/reviewer/peerform', { application_no, comments, marks, reviewer_crn, status },{withCredentials:true})
                    .then((res) => {
                        if (res.data.msg === "success") {
                            toast.success("successful");
                            setPeerFormData(res.data.peerFormData);
                            // console.log(res.data.peerFormData);
                            flag = true;
                            // e.target.reset();
                            navigate('/reviewerdashboard')
                        }
                        console.log(peerFormData);
                    })
            }
            else{
        ...
        <>
            ...
            <div className="container d-flex my-5 p-0" style={{ height: "60vh" }} >
                <div className="container  w-50 d-flex align-items-center justify-content-evenly flex-column rounded p-2" style={{ backgroundColor: "#caf0f8" }}>
                    ...
                    <h2 className="fw-bolder mb-3 text-center text-dark">Peer Screening</h2>
                    ...
                    <div className="container d-flex ">
                        ...
                        <div className="col">{data.status > 0 ? "Recommend" : data.status === -1 ? "Not Recommend" : "No Completed"}</div>
                    </div>
                    <div className="container d-flex ">
                ...
                <div className="container w-25 p-3 rounded" style={{ backgroundColor: "#caf0f8" }} >
                    <div className=" text-center  rounded-3" >
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-floating my-3">
                                <input onChange={(e) => setMarks(e.target.value)} type="number" className="form-control" id="floatingMarks"  placeholder={peerFormData.marks}/>
